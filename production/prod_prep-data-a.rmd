---
title: "Data preparation: Pilot study - Facebook"
...
long01<- melt(wide, id.vars = "ResponseId",
              ...
              value.name = c("taxperc","taxjust","deck_vig"))
...
```{r eval=FALSE, include=FALSE}
sjPlot::view_df(vig_resp,show.frq = T, show.prc = T, show.na = T)
```
...
```{r}
kable(rbind(taxperc= quantile(long01$taxperc, probs = c(0.90,0.91,0.92,0.93,0.94,0.95,0.96,0.97,0.98,0.99,1.00),na.rm = T),
            taxjust= quantile(long01$taxjust, probs = c(0.90,0.91,0.92,0.93,0.94,0.95,0.96,0.97,0.98,0.99,1.00),na.rm = T)),
      caption = "Percentiles 90 a 100 para taxper y taxjust")
```
...
```{r, fig.width=18, fig.cap="Missing en tax justo"}
...
screenreg(list(m03.me,m03.me.1,m03.me.2,m03.me.3,m04.me,m04.me.1,m04.me.2,m04.me.3),
          custom.model.names = c("pertax.0","pertax.0","pertax.0","justax.0","justax.0","justax.0"),
          single.row = T)
```
...
```{r, fig.height=10,fig.width=10, fig.cap="Resultados para tax percibido"}
...
Vemos que en el caso de `taxpec`, las respuestas $0 son un 26,32%, mientras que en `taxjust` son un un 37,57%. Como vimos anteriormente en los modelos regresión, las respuesta no son independientes de los ingresos de la viñeta. Por tanto, se recomienda mantener estos valores. 
...
```{r}
vig_resp <- vig_resp %>% filter(taxperc < 1855000 & taxjust< 1290000)
```
...
```{r}
save(vig_resp,file = "input/data/proc/vig_resp-a.RData")
```
