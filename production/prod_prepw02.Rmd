...
```{r}
 ...
 pacman::p_load(tidyverse,haven,sjlabelled,stringi,sjPlot) # librerias
# load(file = "input/data/proc/factorial_ola1.RData")
data01 <- read_spss(path = "input/data/original/factorial_ola2.sav")
load(file = "input/data/proc/vig_dim.RData")
data01 <- data01 %>% filter(consen==1) %>% data.frame()

data01 %>% dplyr::select(FL_191_DO_11:FL_58_DO_1012) %>% names()
...
decks01<- data01 %>% dplyr::select(ResponseId,"finished"=Finished,
                            starts_with("p_a_"),
                            starts_with("p_b_"),
...
decks03 <- left_join(select(decks01,-nam01),decks02)
names(decks03)
...
table(long01$taxperc<0)
long01$taxperc <- ifelse(long01$taxperc<0,yes = NA,long01$taxperc)
summary(long01$taxperc)
...
table(long01$taxjust<0)
long01$taxjust <- ifelse(long01$taxjust<0,yes = NA,long01$taxjust)
summary(long01$taxjust)
...
long02<- 
  left_join(long01,data01[,c("ticket","ResponseId","Finished","sexo","edad",
                             ...
                             "merit_perc_effort","merit_perc_talent",
...
vig_resp <- vig_resp %>% var_labels(respondeid = "Id Respondent",
                                    ...
                                    merit_perc_effort = "Perceived meritocracy"
                                    )
...
```{r}
vig_respw02 <-vig_resp 
view_df(vig_respw02,show.type = T)
skimr::skim(vig_respw02)
vig_respw02<- vig_respw02[which(vig_respw02$folio_encuestado_1!=""),]
save(vig_respw02,file = "input/data/proc/vig_respw02.RData")
```
